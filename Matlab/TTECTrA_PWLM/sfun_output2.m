...
block.OutputPort(1).Dimensions = size(y,1);
%---
...
C    = block.DialogPrm(6).Data;     % C matrices
D    = block.DialogPrm(7).Data;     % D matrices
...
% calculate y as a weighted sum of the linearized model outputs
y = 0;
for i=1:1:s
    idx_C = ((i-1)*n+1):i*n;
    idx_D = ((i-1)*m+1):i*m;
    
    y = w(i) * ( y_tr(:,i) + C(:,idx_C)*(x - x_tr(:,i)) + D(:,idx_D)*(u - u_tr(:,i))) + y;
end

block.OutputPort(1).Data = y;    % put outputs at output port
%---